...
const defaultToolbar = (ref, id) => (
    <trix-toolbar ref={ref} id={id}>
...
const Toolbar = forwardRef(({ id, toolbar }, ref) => {
    const toolbars = {
        small: smallToolbar,
        medium: mediumToolbar,
        full: defaultToolbar,
    };
    if (toolbar in toolbars) {
        return toolbars[toolbar](ref, id);
    }
    return defaultToolbar(ref, id);
});
Toolbar.displayName = 'Toolbar';

function wordCount(text) {
    return text.split(/\W/).filter((w) => w.length > 0).length;
}
...
export default function HtmlEditor({ component, name, title = '', helperText = '', height = 2, maxLength, initialValue }) {
    ...
    const handleEditorReady = (e) => {
        // console.log(name, 'editor ready', e);
        e.element.addEventListener('keydown', function (event) {
            if (event.key === 'Enter') {
            ...
            <Stack direction='column'>
                ...
                <Box maxHeight='20vh'>
                    <TrixEditor
                        ...
                        toolbar={toolbarId}
                        className={(height < 5) ? 'trix-user-text-short' : 'trix-user-text-full'}
