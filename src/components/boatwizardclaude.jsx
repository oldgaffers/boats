  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-lg">
      ...
      <div className="flex justify-between">
        ...
        >
          <ArrowBackIosIcon className="w-4 h-4 mr-1" />
          Previous
        </button>

        <div className="flex space-x-3">
          {/* Skip button - only show on preferences step (index 1) */}
          {currentStep === 1 && (
            <button
              onClick={handleSkip}
              className="px-4 py-2 text-gray-600 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition-colors"
            >
              Skip Notifications
            </button>
          )}
          ...
          {currentStep === schema.steps.length - 1 ? (
            ...
            >
              Next
              <ArrowForwardIosIcon className="w-4 h-4 ml-1" />
            </button>
          )}
        </div>
      </div>
